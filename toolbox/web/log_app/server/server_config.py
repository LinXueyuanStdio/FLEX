...
from .utils import colored_string
...
def read_server_config(config_path):
    ...
    remote_log_servers = read_list_from_config(config, "data_settings", "remote_log_servers", ",")
    configs['remote_log_servers'] = list(remote_log_servers)
    if len(config.get('data_settings', 'filter_condition')) != 0:
        ...
        for key in list(configs['filter_condition'].keys()):
            ...
            if not isinstance(configs['filter_condition'][key], (str, list, numbers.Number)):
                print(colored_string("Unsupported type found in filter_condition in `{}`.".format(key), 'red'))
                delete = True
