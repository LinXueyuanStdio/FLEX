...
def refresh_table():
    ...
    log_agent = all_data['log_agent']
    try:
        new_logs = log_agent.read_logs(all_data['deleted_rows'])
        # 删除不满足条件的
        if len(new_logs) == 0:
            ...
            return jsonify(status='success',
                           msg=f'Update successfully, {len(updated_logs)} log have updates, {len(new_logs)} newly added.',
