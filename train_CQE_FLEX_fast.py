...
from dataloader import TestDataset, QueryStructureScoringDataset
    ...
    def forward_FLEX(self, positive_sample, negative_sample, subsampling_weight,
        ...
        if type(negative_sample) != type(None):
            ...
            if len(all_feature) > 0:
                ...
                negative_scores = self.scoring(negative_feature, all_feature, all_logic)
            else:
...
class MyExperiment(Experiment):
    ...
    def __init__(self, output: OutputSchema, data: ComplexQueryData,
        ...
        for query_structure in train_queries_answers:
            self.log(query_name_dict[query_structure] + ": " + str(len(train_queries_answers[query_structure])))

        train_structure_to_dataloader = {}
        for key in train_queries_answers:
            queries_answers = train_queries_answers[key]
            train_structure_to_dataloader[key] = DataLoader(
                QueryStructureScoringDataset(queries_answers, nentity, negative_sample_size),
        ...
        for step in range(start_step, max_steps):
            self.log(f"epoch {step + 1}")
            model.train()
            log_loss = self.train(model, opt, train_structure_to_dataloader, step, train_device)
            if (step + 1) % 10 == 0:
                self.metric_log_store.add_loss(log_loss, step + 1)
    ...
    def train(self, model, optimizer, train_structure_to_dataloader, step, device="cuda:0"):
        model.to(device)
        model.train()
        log_loss = defaultdict(list)
        for structure in train_structure_to_dataloader:
            train_dataloader = train_structure_to_dataloader[structure]
            progbar = Progbar(max_step=len(train_dataloader))
            for step, (queries, positive_sample, negative_sample, sampling_weight) in enumerate(train_dataloader):
                optimizer.zero_grad()

                queries = queries.to(device)
                positive_sample = positive_sample.to(device)
                negative_sample = negative_sample.to(device)
                sampling_weight = sampling_weight.to(device)

                positive_score, negative_score = model.single_forward(structure, queries, positive_sample, negative_sample)

                negative_score = F.logsigmoid(-negative_score).mean(dim=1)
                positive_score = F.logsigmoid(positive_score).squeeze(dim=1)
                positive_sample_loss = - (sampling_weight * positive_score).sum() / sampling_weight.sum()
                negative_sample_loss = - (sampling_weight * negative_score).sum() / sampling_weight.sum()

                loss = (positive_sample_loss + negative_sample_loss) / 2
                loss.backward()
                optimizer.step()
                progbar.update(step + 1, [("step", step + 1), ("loss", loss.item()), ("structure", query_name_dict[structure])])
                log_loss[query_name_dict[structure]].append(loss.item())
        return {k: np.mean(v) for k, v in log_loss.items()}
...
def main(dataset, name,
    ...
    data.load_for_scoring_all(evaluate_union, tasks)
