...
class MyExperiment(Experiment):
    ...
    def __init__(self, output: OutputSchema, data: ComplexQueryData,
        ...
        train_path_iterator = SingledirectionalOneShotIterator(DataLoader(
            ScoringAllDataset(train_path_queries, nentity, train_answers),
            batch_size=batch_size,
            shuffle=True,
            num_workers=cpu_num,
            collate_fn=ScoringAllDataset.collate_fn
        ))
        if len(train_other_queries) > 0:
            train_other_queries = flatten_query(train_other_queries)
            train_other_iterator = SingledirectionalOneShotIterator(DataLoader(
                ScoringAllDataset(train_other_queries, nentity, train_answers),
                batch_size=batch_size,
                shuffle=True,
                num_workers=cpu_num,
                collate_fn=ScoringAllDataset.collate_fn
    ...
    def train(self, model, optimizer, train_iterator, step, device="cuda:0"):
        ...
        answer, batch_queries, query_structures = next(train_iterator)
        ...
        predict, _ = model.forwardScoringAll(batch_queries_dict, batch_idxs_dict)
        loss = model.loss(predict, answer)
        loss.backward()
        optimizer.step()
        log = {
            'loss': loss.item(),
...
@click.option("--name", type=str, default="SamE", help="Name of the experiment.")
