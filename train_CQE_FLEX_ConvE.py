...
from toolbox.nn.ConvE import CoreConvE
...
class Projection(nn.Module):
    def __init__(self, hidden_dim):
        ...
        self.core = CoreConvE(2 * hidden_dim, input_dropout=0.1, hidden_dropout1=0.1, hidden_dropout2=0.1)

    def forward(self, q_feature, q_logic, r_feature, r_logic):
        h = torch.cat([q_feature, q_logic], dim=-1)
        r = torch.cat([r_feature, r_logic], dim=-1)
        x = self.core(h, r)
...
class MyExperiment(Experiment):
    ...
    def __init__(self, output: OutputSchema, data: ComplexQueryData,
        ...
        self.model_param_store.save_scripts(["train_CQE_FLEX_ConvE.py"])
...
@click.option("--name", type=str, default="FLEX_ConvE", help="Name of the experiment.")
