...
class Union(nn.Module):
    ...
    def forward(self, feature, logic):
        ...
        logic = torch.sum(logic, dim=0) - torch.prod(logic, dim=0)  # it is right when N=2, but it fails when N>2. This exp only exists 2u, so it is right.
        return feature, logic
...
class FLEX(nn.Module):
    def __init__(self, nentity, nrelation, hidden_dim, gamma,
        ...
        self.union = Union(self.entity_dim)
    ...
    def embed_query(self, queries: torch.Tensor, query_structure, idx: int):
        ...
        if all_relation_flag:
            ...
            for i in range(len(query_structure[-1])):
                ...
                idx += 1
        elif query_structure[-1][0] == 'u':
            # 并运算
            # print("union")
            feature_list = []
            logic_list = []
            structures = query_structure[:-1]
            for i in range(len(structures)):  # 把内部每个子结构都嵌入了，再执行 且运算
                q_feature, q_logic, idx = self.embed_query(queries, structures[i], idx)
            ...
            q_feature, q_logic = self.union(stacked_feature, stacked_logic)
            idx += 1  # skip 'u'
        else:
...
@click.option("--name", type=str, default="FLEX_union", help="Name of the experiment.")
