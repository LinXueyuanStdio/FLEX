...
class Projection(nn.Module):
    ...
    def forward(self, q_feature_1, q_feature_2, q_logic, r_feature_1, r_feature_2, r_logic):
        x = torch.cat([q_feature_1 + r_feature_1, q_feature_2 + r_feature_2, q_logic + r_logic], dim=-1)
        for nl in range(1, self.num_layers + 1):
            x = F.relu(getattr(self, "layer{}".format(nl))(x))
        x = self.layer0(x)

        feature_1, feature_2, logic = torch.chunk(x, 3, dim=-1)
        feature_1 = convert_to_feature(feature_1)
        feature_2 = convert_to_feature(feature_2)
        logic = convert_to_logic(logic)
        return feature_1, feature_2, logic
...
class Intersection(nn.Module):
    def __init__(self, dim):
        ...
        self.dim = dim
        self.feature_1_layer_1 = nn.Linear(self.dim * 2, self.dim)
        self.feature_1_layer_2 = nn.Linear(self.dim, self.dim)
        self.feature_2_layer_1 = nn.Linear(self.dim * 2, self.dim)
        self.feature_2_layer_2 = nn.Linear(self.dim, self.dim)

        nn.init.xavier_uniform_(self.feature_1_layer_1.weight)
        nn.init.xavier_uniform_(self.feature_1_layer_2.weight)
        nn.init.xavier_uniform_(self.feature_2_layer_1.weight)
        nn.init.xavier_uniform_(self.feature_2_layer_2.weight)

    def forward(self, feature_1, feature_2, logic):
        # feature: N x B x d
        # logic:  N x B x d
        logits = torch.cat([feature_1, logic], dim=-1)  # N x B x 2d
        feature_attention = F.softmax(self.feature_1_layer_2(F.relu(self.feature_1_layer_1(logits))), dim=0)
        feature_1 = torch.sum(feature_attention * feature_1, dim=0)

        logits = torch.cat([feature_2, logic], dim=-1)  # N x B x 2d
        feature_attention = F.softmax(self.feature_2_layer_2(F.relu(self.feature_2_layer_1(logits))), dim=0)
        feature_2 = torch.sum(feature_attention * feature_2, dim=0)
...
class Negation(nn.Module):
    ...
    def forward(self, feature_1, feature_2, logic):
        logits = torch.cat([feature_1, logic], dim=-1)  # N x B x 2d
        feature_1 = self.feature_1_layer_2(F.relu(self.feature_1_layer_1(logits)))

        logits = torch.cat([feature_2, logic], dim=-1)  # N x B x 2d
        feature_2 = self.feature_2_layer_2(F.relu(self.feature_2_layer_1(logits)))
...
class FLEX(nn.Module):
    def __init__(self, nentity, nrelation, hidden_dim, gamma,
        ...
        self.entity_feature_1_embedding = nn.Parameter(torch.zeros(nentity, self.entity_dim), requires_grad=True)
        self.entity_feature_2_embedding = nn.Parameter(torch.zeros(nentity, self.entity_dim), requires_grad=True)
        self.relation_feature_1_embedding = nn.Parameter(torch.zeros(nrelation, self.relation_dim), requires_grad=True)
        self.relation_feature_2_embedding = nn.Parameter(torch.zeros(nrelation, self.relation_dim), requires_grad=True)
    ...
    def init(self):
        embedding_range = self.embedding_range.item()
        nn.init.uniform_(tensor=self.entity_feature_1_embedding, a=-embedding_range, b=embedding_range)
        nn.init.uniform_(tensor=self.entity_feature_2_embedding, a=-embedding_range, b=embedding_range)
        nn.init.uniform_(tensor=self.relation_feature_1_embedding, a=-embedding_range, b=embedding_range)
        nn.init.uniform_(tensor=self.relation_feature_2_embedding, a=-embedding_range, b=embedding_range)
    ...
    def forward_FLEX(self, positive_sample, negative_sample, subsampling_weight,
        ...
        all_idxs, all_feature_1, all_feature_2, all_logic = [], [], [], []
        all_union_idxs, all_union_feature_1, all_union_feature_2, all_union_logic = [], [], [], []
        for query_structure in batch_queries_dict:
            # 用字典重新组织了嵌入，一个批次(BxL)只对应一种结构
            if 'u' in self.query_name_dict[query_structure] and 'DNF' in self.query_name_dict[query_structure]:
                feature_1, feature_2, logic, _ = self.embed_query(self.transform_union_query(batch_queries_dict[query_structure], query_structure),
                                                                  self.transform_union_structure(query_structure),
                                                                  0)
                all_union_idxs.extend(batch_idxs_dict[query_structure])
                all_union_feature_1.append(feature_1)
                all_union_feature_2.append(feature_2)
                all_union_logic.append(logic)
            else:
                feature_1, feature_2, logic, _ = self.embed_query(batch_queries_dict[query_structure],
                                                                  query_structure,
                                                                  0)
                all_idxs.extend(batch_idxs_dict[query_structure])
                all_feature_1.append(feature_1)
                all_feature_2.append(feature_2)
                all_logic.append(logic)

        if len(all_feature_1) > 0:
            all_feature_1 = torch.cat(all_feature_1, dim=0).unsqueeze(1)  # (B, 1, d)
            all_feature_2 = torch.cat(all_feature_2, dim=0).unsqueeze(1)  # (B, 1, d)
            all_logic = torch.cat(all_logic, dim=0).unsqueeze(1)  # (B, 1, d)
        if len(all_union_feature_1) > 0:
            all_union_feature_1 = torch.cat(all_union_feature_1, dim=0).unsqueeze(1)  # (2B, 1, d)
            all_union_feature_2 = torch.cat(all_union_feature_2, dim=0).unsqueeze(1)  # (2B, 1, d)
            all_union_logic = torch.cat(all_union_logic, dim=0).unsqueeze(1)  # (2B, 1, d)
            all_union_feature_1 = all_union_feature_1.view(all_union_feature_1.shape[0] // 2, 2, 1, -1)  # (B, 2, 1, d)
            all_union_feature_2 = all_union_feature_2.view(all_union_feature_2.shape[0] // 2, 2, 1, -1)  # (B, 2, 1, d)
        ...
        if type(positive_sample) != type(None):
            ...
            if len(all_feature_1) > 0:
                ...
                positive_feature_1 = torch.index_select(self.entity_feature_1_embedding, dim=0, index=positive_sample_regular).unsqueeze(1)
                positive_feature_1 = self.scale(positive_feature_1)
                positive_feature_1 = convert_to_feature(positive_feature_1)
                positive_feature_2 = torch.index_select(self.entity_feature_2_embedding, dim=0, index=positive_sample_regular).unsqueeze(1)
                positive_feature_2 = self.scale(positive_feature_2)
                positive_feature_2 = convert_to_feature(positive_feature_2)

                positive_logit = self.cal_logit(positive_feature_1, positive_feature_2, all_feature_1, all_feature_2, all_logic)
            else:
                positive_logit = torch.Tensor([]).to(self.entity_feature_1_embedding.device)
            ...
            if len(all_union_feature_1) > 0:
                ...
                positive_feature_1 = torch.index_select(self.entity_feature_1_embedding, dim=0, index=positive_sample_union).unsqueeze(1).unsqueeze(1)
                positive_feature_1 = self.scale(positive_feature_1)
                positive_feature_1 = convert_to_feature(positive_feature_1)
                positive_feature_2 = torch.index_select(self.entity_feature_2_embedding, dim=0, index=positive_sample_union).unsqueeze(1).unsqueeze(1)
                positive_feature_2 = self.scale(positive_feature_2)
                positive_feature_2 = convert_to_feature(positive_feature_2)

                positive_union_logit = self.cal_logit(positive_feature_1, positive_feature_2, all_union_feature_1, all_union_feature_2, all_union_logic)
                positive_union_logit = torch.max(positive_union_logit, dim=1)[0]
            else:
                positive_union_logit = torch.Tensor([]).to(self.entity_feature_1_embedding.device)
        ...
        if type(negative_sample) != type(None):
            ...
            if len(all_feature_1) > 0:
                ...
                negative_feature_1 = torch.index_select(self.entity_feature_1_embedding, dim=0, index=negative_sample_regular.view(-1)).view(batch_size, negative_size, -1)
                negative_feature_1 = self.scale(negative_feature_1)
                negative_feature_1 = convert_to_feature(negative_feature_1)
                negative_feature_2 = torch.index_select(self.entity_feature_2_embedding, dim=0, index=negative_sample_regular.view(-1)).view(batch_size, negative_size, -1)
                negative_feature_2 = self.scale(negative_feature_2)
                negative_feature_2 = convert_to_feature(negative_feature_2)

                negative_logit = self.cal_logit(negative_feature_1, negative_feature_2, all_feature_1, all_feature_2, all_logic)
            else:
                negative_logit = torch.Tensor([]).to(self.entity_feature_1_embedding.device)
            ...
            if len(all_union_feature_1) > 0:
                ...
                negative_feature_1 = torch.index_select(self.entity_feature_1_embedding, dim=0, index=negative_sample_union.view(-1)).view(batch_size, 1, negative_size, -1)
                negative_feature_1 = self.scale(negative_feature_1)
                negative_feature_1 = convert_to_feature(negative_feature_1)
                negative_feature_2 = torch.index_select(self.entity_feature_2_embedding, dim=0, index=negative_sample_union.view(-1)).view(batch_size, 1, negative_size, -1)
                negative_feature_2 = self.scale(negative_feature_2)
                negative_feature_2 = convert_to_feature(negative_feature_2)

                negative_union_logit = self.cal_logit(negative_feature_1, negative_feature_2, all_union_feature_1, all_union_feature_2, all_union_logic)
                negative_union_logit = torch.max(negative_union_logit, dim=1)[0]
            else:
                negative_union_logit = torch.Tensor([]).to(self.entity_feature_1_embedding.device)
    ...
    def embed_query(self, queries: torch.Tensor, query_structure, idx: int):
        ...
        if all_relation_flag:
            ...
            if query_structure[0] == 'e':
                # 嵌入实体
                feature_entity_embedding_1 = torch.index_select(self.entity_feature_1_embedding, dim=0, index=queries[:, idx])
                feature_entity_embedding_1 = self.scale(feature_entity_embedding_1)
                feature_entity_embedding_1 = convert_to_feature(feature_entity_embedding_1)

                feature_entity_embedding_2 = torch.index_select(self.entity_feature_2_embedding, dim=0, index=queries[:, idx])
                feature_entity_embedding_2 = self.scale(feature_entity_embedding_2)
                feature_entity_embedding_2 = convert_to_feature(feature_entity_embedding_2)

                logic_entity_embedding = torch.zeros_like(feature_entity_embedding_1).to(feature_entity_embedding_1.device)

                idx += 1

                q_feature_1 = feature_entity_embedding_1
                q_feature_2 = feature_entity_embedding_2
                q_logic = logic_entity_embedding
            else:
                # 嵌入中间推理状态
                q_feature_1, q_feature_2, q_logic, idx = self.embed_query(queries, query_structure[0], idx)

            for i in range(len(query_structure[-1])):
                # negation
                if query_structure[-1][i] == 'n':
                    assert (queries[:, idx] == -2).all()
                    q_feature_1, q_feature_2, q_logic = self.negation(q_feature_1, q_feature_2, q_logic)

                # projection
                else:
                    r_feature_1 = torch.index_select(self.relation_feature_1_embedding, dim=0, index=queries[:, idx])
                    r_feature_1 = self.scale(r_feature_1)
                    r_feature_1 = convert_to_feature(r_feature_1)
                    r_feature_2 = torch.index_select(self.relation_feature_2_embedding, dim=0, index=queries[:, idx])
                    r_feature_2 = self.scale(r_feature_2)
                    r_feature_2 = convert_to_feature(r_feature_2)
                    ...
                    q_feature_1, q_feature_2, q_logic = self.projection(q_feature_1, q_feature_2, q_logic, r_feature_1, r_feature_2, r_logic)
                idx += 1
        else:
            ...
            for i in range(len(query_structure)):  # 把内部每个子结构都嵌入了，再执行 且运算
                q_feature_1, q_feature_2, q_logic, idx = self.embed_query(queries, query_structure[i], idx)
                feature_1_list.append(q_feature_1)
                feature_2_list.append(q_feature_2)
                logic_list.append(q_logic)

            stacked_feature_1 = torch.stack(feature_1_list)
            stacked_feature_2 = torch.stack(feature_2_list)
            stacked_logic = torch.stack(logic_list)

            q_feature_1, q_feature_2, q_logic = self.intersection(stacked_feature_1, stacked_feature_2, stacked_logic)

        return q_feature_1, q_feature_2, q_logic, idx
    ...
    def cal_logit(self, entity_feature_1, entity_feature_2, query_feature_1, query_feature_2, query_logic):
        distance_1 = self.distance(entity_feature_1, query_feature_1, query_logic)
        distance_2 = self.distance(entity_feature_2, query_feature_2, query_logic)
...
@click.option("--name", type=str, default="FLEX_2F", help="Name of the experiment.")
